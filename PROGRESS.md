...
*Last updated: 2026-08-24 · Branch: `main` @ `ba5f14b` · DB schema version: 5 · CI: green (coreDebug + storeRelease)*
...
**Latest batch (2026-08-24):** **Unlock now accepts biometric OR device credential** (fingerprint / face / PIN / pattern / password) — fixes the hard-fail when no biometric is enrolled. Keystore key uses `AUTH_BIOMETRIC_STRONG | AUTH_DEVICE_CREDENTIAL` (API 30+) or a validity window (26–29); **no CryptoObject** (that path is biometric-only); the passphrase is wrapped/unwrapped inside the key's short auth window; `setInvalidatedByBiometricEnrollment` dropped (would void a PIN-unlockable vault). New no-screen-lock path → `NEEDS_DEVICE_LOCK` setup screen → opens security settings, re-checked `onResume`; `BiometricLockScreen` is mode-aware (SETUP / UNLOCK / NEEDS_DEVICE_LOCK). **Omni-present bottom nav** (`ClearStreakBottomBar`) on all five primary areas. **Journal shows every check-in** (dropped the `note_encrypted IS NOT NULL` filter). **FLAG_SECURE capture variant** (`-Pcapture`, debug-only) so store screenshots can be captured. **Settings decluttered** — removed the "unlocked" confirmation card + the "864zeros Core Promise" card; added the reusable **Support/rate card** (Play In-App Review). **Mobile formalized in the factory:** new `LLC-DIV-3-FACTORY/mobile/` category + `mobile/README.md`; structure = **each mobile app is its own sibling repo of `864zeros-llc`** (ClearStreak stays at `C:\dev\clearStreak`), with the factory `/mobile/<slug>/` holding GTM assets + a `codeRepo` pointer so apps plug into the divisions + GTM pipeline. **No `864zeros-mobile` monorepo** (investigated, rejected). RULE-010 (sibling-repo code locality), RULE-011 (omni-present-nav default).
...
Get it from **GitHub → Actions → latest green run → Artifacts → `clearstreak-core-debug-apk`** (unzip → install; the app unlocks with the device's **fingerprint, face, or PIN/pattern/password** — you just need *some* screen lock set; screenshots are blocked by `FLAG_SECURE`, except an opt-in `-Pcapture` debug build).
...
**Go-live features (built 2026-08-20):** (1) **In-app affirmations** — opt-in via a **"Show affirmations"** setting (default off, `Home & Content`); when on, a bottom **Affirmation card with a Random button** appears in each **Reflect** segment (recovery under Recovery, spiritual under Scripture). `AffirmationStore` loads two pillar files `assets/affirmations_recovery.json` + `assets/affirmations_spiritual.json` with pillar-specific random (`randomRecovery()` / `randomSpiritual()`). Purely in-app — no notification permission. *(Superseded the earlier Home bell-banner placement.)* **Content authored 2026-08-20** (see `SESSION_2026-08-20-affirmations.md`): 50 recovery (original 864zeros writing derived from + citing the 1939 Big Book, PD) + 50 spiritual (derived from + citing exact KJV, PD); **no review gates** (author is reviewer). Placeholder set is now retired. *(App bundles copies under `assets/`; keep in sync with `content/affirmations_*.json`.)* (2) **Reusable Support Card** (`ui/components/SupportCard`) — variable quote + 5-star + expandable radical-transparency; flavor-split review launcher (`review/`): store = Play In-App Review, core = deep-link; wired into Settings. (3) **Milestone Coins** (`ui/components/MilestoneCoin`) — ceremonial tier-tinted coins (generic to ANY journey, not AA-specific) replacing the flat achievement pills on Journey Detail; not a score. Coffee/tip element deliberately **dropped** (BuyMeACoffee stays in the Chrome extensions; in-app tips would hit store policy). Discreet **lock-screen** notifications deferred post-launch (permission surface).
